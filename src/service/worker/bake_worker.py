...
class BakeWorker(BaseWorker):
    ...
    def load(self):
        ...
        with ThreadPoolExecutor(thread_name_prefix="load", max_workers=self.max_worker) as executor:
            motion_future = executor.submit(usecase.load_motion, 0, bake_panel.motion_ctrl.path, self.frame.cache_motions)
            model_future = executor.submit(usecase.load_model_no_copy, 0, bake_panel.model_ctrl.path, self.frame.cache_models)
    ...
    def output_log(self):
        bake_panel = self.frame.bake_panel
        output_log_path = os.path.join(get_root_dir(), f"{os.path.basename(bake_panel.output_motion_ctrl.path)}.log")
        # 出力されたメッセージを全部出力
